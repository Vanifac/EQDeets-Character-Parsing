...
Do {
    Do {
        ...
        $Tr, ($Char[$Nam]), $Serv = "$ActiveLogN" -split '_'

        #--Select Server--
        if ( "$ActiveLogN" -like "*project1999*") { $Char[$Ser] = "Blue" }
        Elseif ( "$ActiveLogN" -like "*P1999PVP*" ) { $Char[$Ser] = "Red" }
        Else { ($Char[$Ser]) = ($Serv.Substring(5)).TrimEnd('.txt') }
